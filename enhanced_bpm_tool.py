...
def create_gradio_interface():
    with gr.Blocks(title="Enhanced BPM Workflow Designer", css="""
        .canvas-container { 
    ...
    """) as demo:
        ...
        gr.Markdown("# 🔄 Enhanced BPM Workflow Designer")
        gr.Markdown("Create and manage business process workflows with advanced visual tools")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Group():
                    ...
                    filename = gr.Textbox(label="Filename", value="my_workflow.json")
                    with gr.Row():
        ...
        def update_stats():
            ...
            for status, count in stats['status_counts'].items():
                ...
                html += f'<span class="status-indicator" style="background-color: {color};"></span>{status.title()}: {count}<br>'
        ...
        add_context_btn.click(
            ...
            outputs=[canvas, stats_display, task_context, context_list, task_list, source_task, target_task]
        )
        ...
        connect_btn.click(
            ...
            outputs=[canvas, stats_display]
        )
        
        disconnect_btn.click(
            disconnect_tasks_handler,
            inputs=[source_task, target_task],
            outputs=[canvas, stats_display]
        )
        ...
        load_btn.click(
            load_handler,
            inputs=[filename],
            outputs=[file_status, canvas, stats_display, task_context, context_list, task_list, source_task, target_task]
        )
        ...
        # Initialize display
        demo.load(
            lambda: (update_stats(), *update_dropdowns()),
            outputs=[stats_display, task_context, context_list, task_list, source_task, target_task]
        )
...
if __name__ == "__main__":
    ...
    task1_id = bpm_tool.add_task("Initialize", ctx1_id, status=TaskStatus.SUCCESS)
    task2_id = bpm_tool.add_task("Validate Input", ctx1_id, status=TaskStatus.SUCCESS)
    task3_id = bpm_tool.add_task("Process Data", ctx1_id, status=TaskStatus.DEFAULT)
    
    task4_id = bpm_tool.add_task("Quality Check", ctx2_id, status=TaskStatus.DELAYED)
    task5_id = bpm_tool.add_task("Error Handling", ctx2_id, status=TaskStatus.SKIPPED)
    
    task6_id = bpm_tool.add_task("Generate Report", ctx3_id, status=TaskStatus.DEFAULT)
    task7_id = bpm_tool.add_task("Send Notification", ctx3_id, status=TaskStatus.DEFAULT)
